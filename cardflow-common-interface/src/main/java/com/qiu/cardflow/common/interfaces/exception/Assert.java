...
public class Assert {
    ...
    /**
     * 检查boolean表达式，当检查结果为false时抛出 {@code BusinessException}。
     * 并使用指定的函数获取错误信息返回
     * <pre class="code">
     ...
     * @param errorMsgSupplier 错误抛出异常附带的消息生产接口
     * @throws BusinessException 表达式为 {@code false} 抛出此异常
     */
    public static void state(boolean expression, Supplier<String> errorMsgSupplier) throws BusinessException {
        if (false == expression) {
            throw new BusinessException(errorMsgSupplier.get());
        }
    }
     ...
     * @throws BusinessException 表达式为 {@code false} 抛出此异常
     */
    public static void state(boolean expression, String errorMsgTemplate, Object... params) throws BusinessException {
        if (false == expression) {
            throw new BusinessException(StrUtil.format(errorMsgTemplate, params));
        }
    }
