...
import com.qiu.cardflow.card.model.dto.card.AnkiNoteAddRequest;
import com.qiu.cardflow.card.model.dto.card.AnkiSyncResponse;
import com.qiu.cardflow.card.model.dto.card.CardAddRequest;
import com.qiu.cardflow.card.model.dto.card.CardUpdateRequest;
...
public class CardService {
    ...
    public Boolean createCard(CardAddRequest cardAddRequest) {
        ...
        // 检查牌组是否存在，不存在则创建
        List<String> userGroups = groupService.getUserGroups(RPCContext.getUserId());
        if (!userGroups.contains(card.getGroup())) {
            groupService.addGroup(RPCContext.getUserId(), card.getGroup());
        }
